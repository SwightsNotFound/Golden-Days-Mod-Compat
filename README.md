# Golden-Days-Mod-Compat
***Please note that the Github's description and releases is not always kept up to date. check the [Modrinth](https://modrinth.com/resourcepack/golden-days-compat) page***
The goal of this resource pack is to re-texture many popular mods so they visually match the **Golden Days** aesthetic.

Make sure Golden Days Mod Compat is above Golden Days on the resource pack screen.

### Currently Supported Mods
If there’s a mod you’d like to see supported, please feel free to create a [GitHub issue](https://github.com/SwightsNotFound/Golden-Days-Mod-Compat/issues) requesting it.  
All mods can be toggled using [Respackopts](https://modrinth.com/mod/respackopts).  

- [Aether](https://modrinth.com/mod/aether)  
- [Amendments](https://modrinth.com/mod/amendments)  
- [Axolotl Buckets](https://modrinth.com/mod/axolotl-buckets)  
- [Backport Paintings 1.21](https://modrinth.com/mod/backport-paintings-1.21)
- [Better Copper / Armor and Tools](https://modrinth.com/mod/better-copper-armor-and-tools)  
- [BetterEnd](https://modrinth.com/mod/betterend) - Does not support variant chests
- [Biomes O' Plenty](https://modrinth.com/mod/biomes-o-plenty)  
- [Brewin' And Chewin'](https://modrinth.com/mod/brewin-and-chewin)  
- [Chisels & Bits](https://www.curseforge.com/minecraft/mc-mods/chisels-bits-for-fabric)  
- [Copper Hopper](https://modrinth.com/mod/copper-hopper)  
- [Crate Delight](https://modrinth.com/mod/crate-delight)  
- [Deep Aether](https://modrinth.com/mod/deep-aether)  
- [End Remastered](https://modrinth.com/mod/endrem)  
- [Exline's Copper Equipment](https://modrinth.com/mod/exlines-copper-equipment)  
- [Exposure](https://modrinth.com/mod/exposure)  
- [Farmer’s Delight](https://modrinth.com/mod/farmers-delight-refabricated)  
- [Fast Item Frames](https://modrinth.com/mod/fast-item-frames)  
- [Horseman](https://modrinth.com/mod/horseman)  
- [Joy of Painting](https://modrinth.com/mod/joy-of-painting)  
- [Leaf's Copper Backport](https://modrinth.com/mod/copper-backport)  
- [MmmMmmMmmMmm](https://modrinth.com/mod/mmmmmmmmmmmm)
- [More Delight](https://modrinth.com/mod/more-delight)  
- [Naturalist](https://modrinth.com/mod/naturalist)  
- [Ocean’s Delight](https://modrinth.com/mod/oceans-delight)  
- [Pumpkin Pie Delight](https://modrinth.com/mod/pumpkin-pie-delight)  
- [Quark](https://modrinth.com/mod/quark) - Does not support variant chests
- [Rustic Delight](https://modrinth.com/mod/rustic-delight)  
- [Sawmill](https://modrinth.com/mod/universal-sawmill)  
- [Supplementaries](https://modrinth.com/mod/supplementaries)  
- [Trials Chambers](https://modrinth.com/mod/trials-chambers-backport)
- [Universal Ores](https://modrinth.com/mod/universal_ores)  
- [Vanilla Backport](https://modrinth.com/mod/vanillabackport)
- [Vegan Delight](https://modrinth.com/mod/vegan-delight)

### To-Do List
Status Key: - [🛠] Working on it - [📝] Planning  
Last updated: 11/04/2025 (month/day/year)
- Custom item models for 1.21.4+ - 🛠
- [Advanced Netherite](https://modrinth.com/mod/advanced-netherite) - 🛠
- [Basic Weapons](https://modrinth.com/mod/basicweapons) - 🛠
- [Better Nether](https://modrinth.com/mod/betternether) - 📝
- Better direct programmer art support. - 🛠
- [Bronze](https://modrinth.com/mod/bronze) - 🛠
- [Buzzier Bees](https://modrinth.com/mod/buzzier-bees) - 🛠
- [Chipped](https://modrinth.com/mod/chipped) - 📝
- [Clayworks](https://modrinth.com/mod/clayworks) - 🛠
- [Comforts](https://modrinth.com/mod/comforts) - 📝
- [Create](https://modrinth.com/mod/create) - 📝
- [Expanded Combat](https://modrinth.com/mod/expanded-combat) - 📝
- [Explorer's Compass](https://modrinth.com/mod/explorers-compass) - 🛠
- [End's Delight](https://modrinth.com/mod/ends-delight) - 🛠
- [Friends and Foes](https://modrinth.com/mod/friends-and-foes) - 📝
- [Gravestone Mod](https://modrinth.com/mod/gravestone-mod) - 🛠
- [Immersive Aircraft](https://modrinth.com/mod/immersive-aircraft) - 📝
- [Immersive Engineering](https://modrinth.com/mod/immersiveengineering) - 📝
- [Incubation](https://modrinth.com/mod/incubation) - 🛠
- [inmis](https://modrinth.com/mod/inmis) - 🛠
- [Iron Furnaces](https://modrinth.com/mod/iron-furnaces) - 📝
- [Let's Do Vinery](https://modrinth.com/mod/lets-do-vinery) - 📝
- [Medieval Weapons](https://modrinth.com/mod/medievalweapons) - 📝
- [My Nether's Delight](https://modrinth.com/mod/my-nethers-delight) - 📝
- [Nature's Compass](https://modrinth.com/mod/natures-compass) - 🛠
- [Pet Cemetery](https://modrinth.com/mod/pet-cemetery) - 🛠
- [Spartan Shields](https://modrinth.com/mod/spartan-shields) - 🛠
- [Spartan Weaponry](https://modrinth.com/mod/spartan-weaponry) - 🛠
- [SecurityCraft](https://modrinth.com/mod/security-craft) - 📝
- See if there's a way to solve some Respackopts issues in older 1.20 versions - 🛠
- [Toms Storage](https://modrinth.com/mod/toms-storage) - 📝
- [Traveler's Backpack](https://modrinth.com/mod/travelersbackpack) - 📝
- [Upgrade Aquatic](https://modrinth.com/mod/upgrade-aquatic) - 🛠


### Notes / Known Issues

Variant chests are not supported, so I recommend that if you’re playing with Quirk, you simply turn them off (`QuirkSettings/Building/Variant chests = false`). BetterEnd unfortunately does not have a toggle, so you would need to use something like [CraftTweaker](https://modrinth.com/mod/crafttweaker) to remove the recipes. You may also need to hide the recipes in your recipe viewer if you have one installed.

Currently, custom item models for pressure plates and fence gates do not work on 1.21.4+. I will fix this at some point.

Custom beta-style boat textures used by Golden Days through EMF are not currently supported for modded boats. This is mainly because I don’t fully understand how EMF works, but also Golden Days’ implementation can cause some visual glitches on older versions. For those reasons, this resource pack disables Golden Days’ custom boats by default. You can toggle this option in [Respackopts](https://modrinth.com/mod/respackopts).


---

If you’d like to help improve or fix any of the issues listed above, please feel free to help out on [GitHub](https://github.com/SwightsNotFound/Golden-Days-Mod-Compat) Contributions are always appreciated.

### Credit Where Credit Is Due

**PoeticRainbow** — for creating *Golden Days* as well as the base of this resource pack’s logo.  
[Golden Days Resource Pack](https://modrinth.com/resourcepack/golden-days)

**TheDarkPanther** — for the *Exline's Copper Equipment* textures.  
[Exline's Copper Equipment × Golden Days](https://modrinth.com/resourcepack/exlines-copper-equipment-x-golden-days) • [MIT License](https://opensource.org/license/mit)  
I’ve updated it to use Golden Days’ new copper textures introduced in 1.21.9.  

**bbali and DrBruhMoments** — for some of the textures in the following mods.  
[Reminiscence](https://www.curseforge.com/minecraft/modpacks/reminiscence) • [MIT License](https://opensource.org/license/mit)  
Amendments / Aether / BetterEnd / Deep Aether / End Remastered / Farmer’s Delight / Joy of Painting / Naturalist / Quark / Sawmill / Supplementaries

**H2OTheMadScientist** — for the MmmMmmMmmMmm (Dummmy) item, and the general base of the model, I have simple updated it to work with newer versions of the mod.  
[Programmer-ish Dummy](https://modrinth.com/resourcepack/programmer-ish-dummy) • [CC0-1.0 License](https://choosealicense.com/licenses/cc0-1.0/)
